...
import PTCompleteProfileModal from "@/components/personal_trainer/PTCompleteProfileModal"
import authApi from "@/lib/api/authApi"
import { useEffect } from "react"
import AuthGuard from "@/components/auth/AuthGuard"

export default function PersonalTrainerLayout({ children }) {
  ...
  const checkProfileStatus = async () => {
    try {
      ...
      // Backend now sends check for PT profile if role is PT
      if (res && res.has_profile === false) {
  ...
  const navGroups = [
    ...
    {
      title: "Overview",
      items: [{ href: "/personal_trainer/dashboard", label: "Dashboard", icon: LayoutDashboard }],
    },
    {
      title: "Client Management",
      items: [
        { href: "/personal_trainer/session-notes", label: "Session Notes", icon: ClipboardList },
        { href: "/personal_trainer/requests", label: "Booking Requests", icon: FileCheck },
      ],
    },
    {
      title: "Schedule",
      items: [
        { href: "/personal_trainer/schedule", label: "My Schedule", icon: Calendar },
      ],
    },
    {
      title: "Communication",
      items: [
        { href: "/personal_trainer/chat", label: "Client Messages", icon: MessageSquare },
      ],
    },
  ]
      ...
      <div className="flex min-h-screen bg-[#141414]">
        ...
        >
          <div className="p-6 border-b border-[#282828] flex items-center justify-between sticky top-0 bg-[#141414] z-10">
            {!isCollapsed && <h1 className="text-xl font-semibold text-[#f0f0f0]">Trainer Portal</h1>}
            <button
        ...
        <div className="flex-1 flex flex-col min-h-screen">
          <header className="border-b border-[#282828] bg-[#141414] px-8 py-4 sticky top-0 z-20">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-medium text-[#f0f0f0]">Welcome back, Coach</h2>
                  ...
                  {showProfileMenu && (
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-[#282828] border border-[#282828] rounded-lg shadow-lg z-40 overflow-hidden">
                        <Link
                          href="/personal_trainer/profile"
                          className="w-full flex items-center gap-3 px-4 py-3 border-b border-[#000000] text-[#f0f0f0] hover:bg-[#1E1E1E] transition-colors text-sm"
        ...
        {showCompleteProfile && <PTCompleteProfileModal onComplete={() => setShowCompleteProfile(false)} />}
      </div>
    </AuthGuard>
  )
}
